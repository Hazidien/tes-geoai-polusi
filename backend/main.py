...
def analyze(request: AnalysisRequest):
    ...
    try: return analyze_co(request.model_dump())
    except (ValueError, RuntimeError) as exc: raise HTTPException(status_code=400, detail=str(exc)) from exc
    except Exception as exc: raise HTTPException(status_code=500, detail=f'GEE processing failed: {exc}') from exc
...
def export_geotiff(request: AnalysisRequest):
    ...
    except Exception as exc: raise HTTPException(status_code=500, detail=f'GeoTIFF export failed: {exc}') from exc

@app.post('/api/report/pdf')
def report_pdf(request: AnalysisRequest):
    try:
        result = analyze_co(request.model_dump())
        buffer = io.BytesIO(); doc = SimpleDocTemplate(buffer, pagesize=A4, rightMargin=42, leftMargin=42, topMargin=42, bottomMargin=42)
        styles = getSampleStyleSheet(); story = [Paragraph('WebGIS Air Pollution — Analysis Report', styles['Title']), Spacer(1, 12)]
        rows = [['Module', result['module']], ['Variable', result['variable']], ['Dataset', result['dataset']], ['Period', f"{result['start_date']} → {result['end_date']}"], ['Aggregation', result['aggregation'].title()], ['Statistic', f"{result['value']:.6e} {result['unit']}"], ['Images', str(result['image_count'])]]
        table = Table(rows, colWidths=[110, 350]); table.setStyle(TableStyle([('GRID',(0,0),(-1,-1),0.5,colors.grey),('BACKGROUND',(0,0),(0,-1),colors.whitesmoke),('VALIGN',(0,0),(-1,-1),'TOP'),('PADDING',(0,0),(-1,-1),7)]))
    ...
    except Exception as exc: raise HTTPException(status_code=500, detail=f'PDF report generation failed: {exc}') from exc
...
def upload_geotiff(file: UploadFile = File(...)):
    ...
    data = file.file.read()
    if len(data) > 100 * 1024 * 1024: raise HTTPException(status_code=400, detail='GeoTIFF is too large. Maximum size is 100 MB.')
    file_id = uuid.uuid4().hex; tif_path = UPLOAD_DIR / f'{file_id}.tif'; tif_path.write_bytes(data)
    try:
        with rasterio.open(tif_path) as src:
            if src.crs is None: raise HTTPException(status_code=400, detail='GeoTIFF has no CRS/georeferencing information.')
            ...
            width,height,count,dtype,nodata = src.width,src.height,src.count,src.dtypes[0],src.nodata
            ...
            import numpy as np
            arr=np.asarray(arr,dtype='float32'); finite=arr[np.isfinite(arr)]
        ...
        tif_path.unlink(missing_ok=True); raise
    except Exception as exc:
        tif_path.unlink(missing_ok=True); raise HTTPException(status_code=400, detail=f'Could not read GeoTIFF: {exc}') from exc

@app.get('/api/upload/geotiff/{file_id}/preview')
def geotiff_preview(file_id:str):
    path=UPLOAD_DIR/f'{file_id}.png'
    if not path.is_file(): raise HTTPException(status_code=404, detail='GeoTIFF preview not found.')
    return FileResponse(path,media_type='image/png')

@app.get('/api/upload/geotiff/{file_id}/download')
def geotiff_download(file_id:str):
    path=UPLOAD_DIR/f'{file_id}.tif'
    if not path.is_file(): raise HTTPException(status_code=404, detail='Uploaded GeoTIFF not found.')
