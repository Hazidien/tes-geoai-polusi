...
app=FastAPI(title='WebGIS Remote Sensing',version='0.4.0')
...
def report_pdf(request:AnalysisRequest):
    ...
    try:
        ...
        story=[Paragraph('WebGIS Analisis Polusi Udara',styles['Title']),Paragraph('Carbon Monoxide (CO) — Sentinel-5P',styles['Heading2']),Paragraph('Dikembangkan oleh <b>Hazidien</b>',styles['SmallNote']),Spacer(1,7),Paragraph('Laporan ini menyajikan hasil analisis spasial dan temporal Carbon Monoxide (CO) berdasarkan data Sentinel-5P pada area kajian (AOI) yang dipilih pengguna.',styles['BodyText']),Spacer(1,8),RLImage(str(map_path),width=175*mm,height=112*mm),Paragraph('Peta hasil analisis pada AOI terpilih. Warna menunjukkan nilai relatif CO dari citra Sentinel-5P.',styles['SmallNote']),Spacer(1,7),Paragraph('Parameter Analisis',styles['SectionTitle'])]
        ...
        story += [table,Spacer(1,9),Paragraph('Time Series',styles['SectionTitle']),pdf_chart(series),Spacer(1,7),Paragraph('Penjelasan',styles['SectionTitle']),Paragraph('Analisis menggunakan koleksi Sentinel-5P untuk Carbon Monoxide dengan band <b>CO_column_number_density</b>. Nilai dinyatakan sebagai <b>column number density (mol/m²)</b>. Materi referensi menjelaskan penggunaan Sentinel-5P untuk CO dan pembuatan chart time series berdasarkan nilai rata-rata wilayah kajian.',styles['SmallNote']),Spacer(1,5),Paragraph('<b>Catatan penting:</b> hasil ini merupakan pengukuran kolom atmosfer oleh satelit, bukan pengukuran langsung konsentrasi udara di permukaan dan bukan nilai ISPU.',styles['SmallNote']),Spacer(1,5),Paragraph('Tujuan WebGIS: menyediakan media eksplorasi geospasial untuk memahami pola spasial dan perubahan temporal CO pada AOI secara interaktif, serta menghasilkan GeoTIFF dan laporan PDF.',styles['SmallNote'])]
