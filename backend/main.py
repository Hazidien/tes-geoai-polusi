...
app=FastAPI(title='WebGIS Remote Sensing',version='0.8.1')
...
@app.get('/api/health')
def health(): return {'status':'ok','application':'WebGIS Air Pollution','version':'0.8.1','variables':['SO2','NO2','CO','CH4']}
...
def chart(request:AnalysisRequest):
    validate_request(request);data=payload(request);key=cache_key(data)
    try:
        entry=ANALYSIS_CACHE.setdefault(key,{'request':data});series=entry.get('series')
...
def export_geotiff(request:AnalysisRequest):
    ...
    try:
        ...
        with urllib.request.urlopen(urllib.request.Request(url,headers={'User-Agent':'WebGIS-Air-Pollution/0.8.1'}),timeout=180) as response:data=response.read()
...
def upload_aoi(file:UploadFile=File(...)):
    ...
    try:
        ...
        result=shapefile_zip_to_geojson(raw);return {'success':True,'filename':file.filename,'geometry':result['geometry'],'feature_count':result['feature_count'],'source_epsg':result['source_epsg'],'source':'Shapefile'}
...
def pdf_chart(series,variable,unit):
    ...
    vals=[float(x['value']) for x in series];lo,hi=min(vals),max(vals);pad=(hi-lo)*.08 or max(abs(lo)*.05,1e-8);p.yValueAxis.valueMin=lo-pad;p.yValueAxis.valueMax=hi+pad;p.xValueAxis.labels.fontSize=7;p.yValueAxis.labels.fontSize=7;d.add(p);d.add(String(35,height-9,f'{variable} time series ({unit})',fontSize=8,fillColor=colors.HexColor('#334155')));return d

def fallback_preview(request,result,cfg):
    path=UPLOAD_DIR/f'{uuid.uuid4().hex}_fallback.png';img=Image.new('RGB',(1200,700),'white');draw=ImageDraw.Draw(img);draw.rectangle((40,40,1160,660),outline='#334155',width=3);draw.text((65,65),f'{cfg["name"]} — Sentinel-5P',fill='#0f2742');draw.text((65,95),f'Result: {result["value"]:.6e} {cfg["unit"]}',fill='#334155');draw.text((65,125),'AOI outline / analysis preview',fill='#64748b')
    def rings(geometry):
...
def aoi_bbox(geometry):
    coords=geometry.get('coordinates',[])
    def walk(obj):
        if isinstance(obj,(list,tuple)) and len(obj)>=2 and isinstance(obj[0],(int,float)) and isinstance(obj[1],(int,float)):yield float(obj[0]),float(obj[1])
        elif isinstance(obj,(list,tuple)):
            for item in obj:yield from walk(item)
    pts=list(walk(coords))
    if not pts:raise ValueError('AOI has no coordinates.')
    xs=[p[0] for p in pts];ys=[p[1] for p in pts];padx=max((max(xs)-min(xs))*0.04,0.002);pady=max((max(ys)-min(ys))*0.04,0.002)
    return min(xs)-padx,min(ys)-pady,max(xs)+padx,max(ys)+pady

def get_map_preview(data,result,cfg):
    xmin,ymin,xmax,ymax=aoi_bbox(data['aoi']);width,height=1000,650
    # The same Web Mercator/ArcGIS street basemap family used by the web map is
    # exported for the AOI extent, then the Earth Engine pollutant raster is
    # composited over it. ArcGIS export supports bbox/size/imageSR parameters.
    basemap_url=('https://server.arcgisonline.com/ArcGIS/rest/services/World_Street_Map/MapServer/export?'
                 f'bbox={xmin},{ymin},{xmax},{ymax}&bboxSR=4326&imageSR=4326&size={width},{height}&format=png&transparent=false&f=image')
    with urllib.request.urlopen(urllib.request.Request(basemap_url,headers={'User-Agent':'WebGIS-Air-Pollution/0.8.1'}),timeout=45) as response:base_raw=response.read()
    if not base_raw.startswith(b'\x89PNG'):raise ValueError('Basemap export returned an invalid image.')
    image,_=build_image(data);visual=image.visualize(min=cfg['min'],max=cfg['max'],palette=cfg['palette']).updateMask(image.mask())
    try:
        import ee
        boundary=ee.Image().byte().paint(ee.Geometry(data['aoi']),1,3).visualize(min=0,max=1,palette=['FFFFFF']);visual=visual.blend(boundary)
    except Exception:pass
    thumb=visual.getThumbURL({'region':[xmin,ymin,xmax,ymax],'dimensions':f'{width}x{height}','format':'png'})
    with urllib.request.urlopen(urllib.request.Request(thumb,headers={'User-Agent':'WebGIS-Air-Pollution/0.8.1'}),timeout=90) as response:overlay_raw=response.read()
    if not overlay_raw.startswith(b'\x89PNG'):raise ValueError('Earth Engine returned an invalid map preview.')
    base=Image.open(io.BytesIO(base_raw)).convert('RGBA');overlay=Image.open(io.BytesIO(overlay_raw)).convert('RGBA').resize(base.size)
    if overlay.getextrema()[3][1]==0:raise ValueError('Earth Engine map preview was empty.')
    composed=Image.alpha_composite(base,overlay)
    path=UPLOAD_DIR/f'{uuid.uuid4().hex}_map.png';composed.save(path,'PNG');return path

@app.post('/api/report/pdf')
def report_pdf(request:AnalysisRequest):
    validate_request(request);data=payload(request);key=cache_key(data);map_path=None
    try:
        entry=ANALYSIS_CACHE.setdefault(key,{'request':data});result=entry.get('result') or analyze_pollutant(data);entry['result']=result;series=entry.get('series')
        if series is None:series=build_timeseries(data);entry['series']=series
        ...
        buffer=io.BytesIO();doc=SimpleDocTemplate(buffer,pagesize=A4,rightMargin=15*mm,leftMargin=15*mm,topMargin=14*mm,bottomMargin=14*mm);styles=getSampleStyleSheet();styles.add(ParagraphStyle(name='SmallNote',parent=styles['BodyText'],fontSize=8.5,leading=12,textColor=colors.HexColor('#475569')));styles.add(ParagraphStyle(name='SectionTitle',parent=styles['Heading2'],fontSize=13,leading=16,textColor=colors.HexColor('#0f2742'),spaceBefore=8,spaceAfter=6))
        story=[Paragraph('WebGIS Air Pollution Analysis',styles['Title']),Paragraph(cfg['name']+' — Sentinel-5P',styles['Heading2']),Paragraph('Developed by <b>Hazidien Ramadhan Utomo</b>',styles['SmallNote']),Spacer(1,7),Paragraph('This report presents the spatial and temporal analysis of the selected Sentinel-5P pollutant over the user-selected Area of Interest (AOI).',styles['BodyText']),Spacer(1,8),RLImage(str(map_path),width=175*mm,height=112*mm),Paragraph('Map extent follows the selected AOI. The basemap is shown beneath the colored Sentinel-5P pollutant layer.',styles['SmallNote']),Spacer(1,7),Paragraph('Analysis Parameters',styles['SectionTitle'])]
        rows=[['Module','Air Pollution'],['Variable',cfg['name']],['Dataset',cfg['dataset']],['Band',cfg['band']],['Period',f"{result['start_date']} → {result['end_date']}"],['Spatial aggregation',result['aggregation'].title()],['Time-series interval',request.interval.title()],['Images',str(result['image_count'])],['Statistic',f"{result['value']:.6e} {result['unit']}"]];table=Table(rows,colWidths=[48*mm,125*mm]);table.setStyle(TableStyle([('GRID',(0,0),(-1,-1),.4,colors.HexColor('#cbd5e1')),('BACKGROUND',(0,0),(0,-1),colors.HexColor('#f1f5f9')),('VALIGN',(0,0),(-1,-1),'TOP'),('PADDING',(0,0),(-1,-1),6),('FONTNAME',(0,0),(0,-1),'Helvetica-Bold')]))
